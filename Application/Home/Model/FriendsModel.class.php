...
class FriendsModel extends Model
{
    public function friendList(){
        $db = new Model();
        $user = session('user');
        $sql ="select u.id as id,u.username as username from (select * from friend_record where ownerid=".$user['id'].") fr join user u on fr.friendid = u.id ";
        $result = $db->query($sql);
    ...
    public function friendFansList(){
        $db = new Model();
        $user = session('user');
        $sql ="select u.id as id,u.username as username from (select * from friend_record where friendid=".$user['id'].") fr join user u on fr.ownerid = u.id ";
        $result = $db->query($sql);
