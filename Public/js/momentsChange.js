            ...
            if(data.state == true){
                ...
                if(object !=null) {
                    for (var i = 0; i < object.length; i++) {
                        var tem = makeTheHtmlElement(object[i].id, object[i].ownerid, object[i].ownername, object[i].createdate, object[i].title,object[i].isMy,i);
...
function  detailButton(id) {                                   //要实现查看某一个文件啊TAT
    var deleteId =  id;
    var point =  $(('#'+deleteId)).parent().parent().parent().children(':hidden').html();
    swal({
        title: "确定删除？",
